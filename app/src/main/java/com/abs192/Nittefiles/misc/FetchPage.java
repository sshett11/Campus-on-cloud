...
public class FetchPage extends AsyncTask<String, Void, String> {
	...
	public FetchPage(FragTeacher frag) {
		this.frag1 = frag;
		this.context = this.frag1.getActivity();
		x=1;
	}
	...
	protected void onPreExecute() {
		...
		{
			super.onPreExecute();
			this.frag1.pb.setVisibility(View.VISIBLE);
			FragTeacher.searching = true;
		}
	}
	...
	protected void onPostExecute(String result) {
       if(x==0)
	   {
		   if (!result.equals("error")) {
			   ...
			   try {
				   ((MainActivity) this.frag.getActivity()).setTitle("\t"+ title(FragHome.presentAddress));
		...
		else if(x==1)
	   {
		   if (!result.equals("error"))
		   {
			   FragTeacher.searching = false;
			   this.frag1.pb.setVisibility(View.INVISIBLE);
			   ParseData pd = new ParseData(result);
			   ...
			   data = pd.getData();
			   url = pd.getUrl();
			   frag1.lv.removeAllViewsInLayout();

			   populateList(data, url);
			   try {
				   ((MainActivity) this.frag1.getActivity()).setTitle("\t"+ title(FragTeacher.presentAddress));
			   } catch (NullPointerException e)
			   {
				   e.printStackTrace();
			   }
		...
		else if(x==1)
	   {
		   if (data != null && url != null) {
			   ...
			   for (int i = 0; i < data.size(); i++) {
				   String d = data.get(i).trim().replaceAll("%20", " ");
				   RowItem item = new RowItem(1, getImage(d), d, url.get(i),
						   checkIfOfflinePresent(FragTeacher.presentAddress
								   + url.get(i)));
				   rowItems.add(item);
			   }
	...
	private boolean checkIfOfflinePresent(String p) {
		...
		File f = new File(SDCardRoot, p.replace(
				"http://192.168.201.1:81/", "/NITTEFILES/"));
